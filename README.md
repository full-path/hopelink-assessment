...
No backend, no build-time secrets, no database: the CSV in `/data` is normalized once at build
time into a typed JSON file that the static frontend reads.
...
| `npm test`                        | Runs the normalization script's unit tests (Vitest).                           |
...
There is no in-app editing UI by design (see CLAUDE.md, Section 10). To reflect a change in
agency intake practice:
...
An agency name not already in `CANONICAL_AGENCIES` (`scripts/build-data.ts`) fails the build rather
than being silently mis-parsed — add it to that table (with any alias spellings you want tolerated)
...
- **Data contract.** `src/data/types.ts` is the single interface between the build-time
  normalization script and the frontend. Application code never sees a raw CSV row.
